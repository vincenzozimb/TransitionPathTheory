...
from inverse.inv_init import X, Y, V, prob, beta, R_bol, P_bol


# Create folder for images, if it does not already exist
if not os.path.exists('inverse/images'):
   os.makedirs('inverse/images')


# useful variables
T_bol = R_bol | P_bol
Vmax = 200
V_plot = V[V > Vmax] = np.nan
...
plt.tight_layout()
plt.savefig("inverse/images/p&g.png")


# import committor
q = np.load("inverse/data.npy")
...
plt.tight_layout()
plt.savefig("inverse/images/q&m.png")
...
plt.tight_layout()
plt.savefig("inverse/images/J_tpt.png")